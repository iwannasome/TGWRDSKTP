...
Ручной workflow способен собрать проверочные неподписанные artifacts. Его Windows/macOS-шаги не получают даже пустых signing-переменных, поэтому electron-builder не может принять каталог проекта за путь к сертификату. Теговый `stable`-релиз работает fail-closed: без полного набора сертификатов job `Проверить сертификаты stable-релиза` завершается ошибкой, нативная matrix не запускается и черновик GitHub Release не создаётся.
