...
export default function Slide09FastestReplyPerson({ report, period }: SlideCommonProps): JSX.Element {
  const p = getPeriod(report, period)
  const champ = getReplyChampion(p, 'who_you_reply_fastest')

  return (
    <SlideFrame kicker="Replies" title="Кому отвечаешь быстрее всех" subtitle="Медианное время ответа на сообщения других." >
      <div className="flex h-full flex-col justify-center">
        ...
        >
          ...
          <div className="mt-6 text-[90px] font-bold leading-none">
            <span className="tgwr-gradient-text">{champ ? formatSecondsHuman(champ.seconds) : '—'}</span>
          </div>
          <div className="mt-4 text-[16px] text-[rgba(var(--tgwr-muted-rgb),0.92)]">медиана ответа</div>
