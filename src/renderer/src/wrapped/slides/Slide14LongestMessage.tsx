...
export default function Slide14LongestMessage({ report, period }: SlideCommonProps): JSX.Element {
  const p = getPeriod(report, period)
  const m = getLongestMessage(p)

  return (
    <SlideFrame kicker="Text" title="Самое длинное сообщение" subtitle="Длиннее — не значит лучше, но это забавно." >
        ...
        >
          <div className="flex items-start justify-between gap-6">
            <div>
              ...
              <div className="mt-2 text-[28px] font-bold text-slate-50">{m ? formatInt(m.length) : '—'} chars</div>
            </div>
          ...
          {!m ? (
            <div className="mt-6 text-[13px] text-[rgba(var(--tgwr-muted-rgb),0.85)]">Нужны исходящие текстовые сообщения.</div>
          ) : null}
        </motion.div>
      </div>
    </SlideFrame>
