...
export default function Slide06NightRatio({ report, period }: SlideCommonProps): JSX.Element {
  const p = getPeriod(report, period)
  const night = getNightRatio(p)
  const total = getTotalMessages(p)

  return (
    <SlideFrame kicker="Late hours" title="Ночные сообщения" subtitle="00:00–05:59 по MSK." >
      <div className="flex h-full flex-col justify-center">
        <div className="grid grid-cols-2 gap-8">
          ...
          >
            ...
            <div className="mt-4 text-[16px] text-[rgba(var(--tgwr-muted-rgb),0.9)]">доля ночных сообщений</div>
          </motion.div>
          ...
          >
            ...
            <div className="mt-4 text-[76px] font-bold leading-none text-slate-50">{formatInt(night.count)}</div>
