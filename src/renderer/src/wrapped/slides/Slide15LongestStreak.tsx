...
export default function Slide15LongestStreak({ report, period }: SlideCommonProps): JSX.Element {
  const p = getPeriod(report, period)
  const streak = getLongestStreak(p)
  const personStreak = getLongestPersonStreak(report, period)

  return (
    <SlideFrame kicker="Consistency" title="Самая длинная серия" subtitle="Дни подряд с хотя бы 1 сообщением." >
        ...
        >
          ...
          {personStreak ? (
            <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5">
              <div className="text-[11px] uppercase tracking-[0.22em] text-white/50">Самый длинный стрик с человеком</div>
