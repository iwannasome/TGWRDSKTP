...
export default function Slide07TopPersonMessages({ report, period }: SlideCommonProps): JSX.Element {
    ...
    <SlideFrame kicker="People" title="Топ персона" subtitle="С кем больше всего сообщений." >
      <div className="flex h-full flex-col justify-center">
        ...
        >
          ...
          <div className="mt-4 text-[16px] text-[rgba(var(--tgwr-muted-rgb),0.92)]">сообщений всего</div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-6">
            <div className="rounded-3xl border border-white/10 bg-white/5 px-6 py-5">
              ...
              <div className="mt-2 text-[26px] font-bold text-slate-50">{formatInt(sent)}</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-white/5 px-6 py-5">
              <div className="text-xs font-semibold uppercase tracking-[0.38em] text-[rgba(var(--tgwr-muted-rgb),0.75)]">
                Received
              </div>
              <div className="mt-2 text-[26px] font-bold text-slate-50">{formatInt(received)}</div>
            </div>
          </div>
