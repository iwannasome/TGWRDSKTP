...
export default function Slide12EmojiTop({ report, period }: SlideCommonProps): JSX.Element {
    ...
    <SlideFrame kicker="Emoji" title="Твои эмодзи" subtitle="Топ эмодзи (в тексте и sticker_emoji)." >
      <div className="flex h-full flex-col justify-center">
        <div className="rounded-[44px] border border-white/10 bg-white/5 p-10">
          {emojis.length === 0 ? (
            <div className="text-[16px] text-[rgba(var(--tgwr-muted-rgb),0.9)]">Пока пусто — эмодзи не найдены.</div>
          ) : (
            <div className="grid grid-cols-4 gap-6">
              {emojis.map((e, idx) => (
                ...
                >
                  ...
                  <div className="mt-2 text-[18px] font-bold text-slate-50">{formatInt(e.count)}</div>
                </motion.div>
              ))}
            </div>
          )}
